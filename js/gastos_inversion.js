...
function load(page){
	...
	$("#loader").fadeIn('slow');
	$.ajax({
		url:'./ajax/GastosInversion/buscar_gastosinversion.php?action=ajax&page='+page+'&q='+q,
...
function eliminar (id){
	...
	swal({
		...
		confirmButtonText: 'Si, eliminar Dato!'
	}).then(function(result){
	if (result.value) {
		$.ajax({
			type: "GET",
		    url: "./ajax/GastosInversion/buscar_gastosinversion.php",
		    data: "id="+id,"q":q,
			beforeSend: function(objeto){
...
$( "#guardar_gastoinversion" ).submit(function( event ) {
$('#guardar_datos').attr("disabled", true);
 var parametros = $(this).serialize();
	 $.ajax({
			type: "POST",
			url: "ajax/GastosInversion/nuevo_gastosinversion.php",
			data: parametros,
			 beforeSend: function(objeto){
